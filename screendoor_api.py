...
monthly_resolution_sum = screendoor_df.groupby(['request_type_grouped', pd.Grouper(key='submitted_at', freq='M')])['update_time'].sum()
monthly_resolution_sum = monthly_resolution_sum.reset_index()
...
#2. calculate number of monthly submissions request type 
monthly_resolution_count = screendoor_df.groupby(['request_type_grouped', pd.Grouper(key='submitted_at', freq='M')])['update_time'].count()
monthly_resolution_count = monthly_resolution_count.reset_index()
monthly_resolution_count = monthly_resolution_count.rename(index=str,columns={'update_time':'update_time_count'})
#3. join monthly_resolution_sum and monthly_resolution_count to get average monthly resolution time per inquiry type
